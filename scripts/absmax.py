...
class ABSMaxQuantization:
    ...
    def _download_model(self):
        self.model_path = hf_hub_download(self.model_name, filename = "model.safetensors") 

    def _perform_absmax(self, tensor):
        '''
        b - number of bytes to quantize to(8)
        alpha - highest absolute value
        s = ( 2 ^ (b-1) - 1) / alpha
        tensor_q = round(s * x)

        tensor_normal = tensor_q / s
        '''
...
if __name__ == '__main__':
    mapper = ABSMaxQuantization("gpt2")
